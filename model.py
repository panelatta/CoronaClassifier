...
class DNASequenceClassifier(nn.Module):
    def __init__(self):
        ...
        self.cnn1 = CNNDecoder(in_channels=4, out_channels=16, kernel_size=3)   # 3*1 CNN
        self.cnn2 = CNNDecoder(in_channels=4, out_channels=32, kernel_size=2)   # 2*1 CNN
        self.cnn3 = CNNDecoder(in_channels=4, out_channels=64, kernel_size=2)   # 2*1 CNN

        self.transformer = Transformer(input_dim=64, nheads=8, dim_feedforward=256, num_layers=3)
